...
#include <QScreen>
#include <QMainWindow> // Let's get current screen here
...
{
    // Iterate over all TopLevel
    foreach (QWidget *w, qApp->topLevelWidgets()) {
        // If is the QMainWindow...
        if (QMainWindow* mainWin = qobject_cast<QMainWindow*>(w)){
            // ..extract the screen from QMainWindow
            return ::mmToInches(_mm) * (_x ? mainWin->screen()->logicalDotsPerInchX() : mainWin->screen()->logicalDotsPerInchY());;
        }
    }
    return ::mmToInches(_mm) * 96;
}
